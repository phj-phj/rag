...
所有上传的文件都在这里。顶部有搜索框，左侧可以按分类和标签筛选。每个文件卡片显示标题、类型、上传者。点击文件可以进入阅读器，适配部分移动端。
...
点击任意文档进入阅读模式。PDF 文件直接全部展示，不会因为上下文的页数导致知识点分散，Word 和纯文本文件直接渲染在页面上。文档上方可以看到文档的基本信息（上传者、大小）。
...
小文档（几十 KB）几秒完成。大 PDF（几十 MB）因为需要提取文字、切块、逐块生成题目，可能需要一两分钟。本地使用的话，处理进度可以在backend/log中查看。
...
访问网站:http://47.95.112.34/，注册一个账号，从自带的示例文档开始试用，或者直接上传自己的学习资料。

上传文档后系统会自动处理——切块、分类、出题。稍等片刻就能在"已收录"和"每日训练"页面看到题目了。

‘也可以在backend/logs这里看到实时的日志报告’
...
详细的安装、配置、API 接口说明请查看同一目录下的 [SETUP.md](SETUP.md)。
